...
public class JdbcConnection {
    ...
    private static String getTableByLetter(String word) {
        ...
        if(first =='á' || first =='ä'|| first =='à') return "letra_a";
       
        if(first =='é' || first =='ë'|| first =='è') return "letra_e";
        if(first =='í' || first =='ï'|| first =='ì') return "letra_i";
        if(first =='ó' || first =='ö'|| first =='ò') return "letra_o";
        if(first =='ú' || first =='ü'|| first =='ù') return "letra_u";
        return "letra_" + first;
    }

    static String url = "jdbc:mysql://192.168.1.6:3306/" + db; // table details
    ...
    public static void update(String word, String definition) throws SQLException, ClassNotFoundException {
        ...
        try (Connection con = DriverManager.getConnection(url, database_user, pass)) {
          ...
          //  st.execute(query_select);
            ResultSet rs = st.executeQuery(QUERY.SELECT(word)); // Execute query
